...
class AwsS3ResolverFactory implements ResolverFactoryInterface
{
    ...
    public function create(ContainerBuilder $container, $resolverName, array $config)
    {
        ...
        $resolverDefinition = new DefinitionDecorator('anezi_imagine.cache.resolver.prototype.aws_s3');
        ...
        if ($config['proxies']) {
            $proxiedResolverId = 'anezi_imagine.cache.resolver.'.$resolverName.'.proxied';

            $container->setDefinition($proxiedResolverId, $resolverDefinition);

            $proxyResolverDefinition = new DefinitionDecorator('anezi_imagine.cache.resolver.prototype.proxy');
        ...
        if ($config['cache']) {
            $cachedResolverId = 'anezi_imagine.cache.resolver.'.$resolverName.'.cached';

            $container->setDefinition($cachedResolverId, $container->getDefinition($resolverId));

            $cacheResolverDefinition = new DefinitionDecorator('anezi_imagine.cache.resolver.prototype.cache');
