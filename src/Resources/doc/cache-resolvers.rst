...
container and apply the ``anezi_imagine.cache.resolver`` tag to it (example here
in XML):

.. code-block:: xml

    <service id="acme_imagine.cache.resolver.my_custom" class="Acme\ImagineBundle\Imagine\Cache\Resolver\MyCustomCacheResolver">
        <tag name="anezi_imagine.cache.resolver" resolver="my_custom_cache" />
...
``Anezi\ImagineBundle\Imagine\Cache\Resolver\WebPathResolver``.
